...
func getSplunkStatefulSet(client splcommon.ControllerClient, cr splcommon.MetaObject, spec *enterprisev1.CommonSplunkSpec, instanceType InstanceType, replicas int32, extraEnv []corev1.EnvVar) (*appsv1.StatefulSet, error) {
	...
	if len(spec.AppFrameworkConfig.AppSources) > 0 {
		...
		statefulSet.Spec.Template.Spec.Volumes = append(statefulSet.Spec.Template.Spec.Volumes, initVol)
		...
		for i, appSrc := range spec.AppFrameworkConfig.AppSources {
			// Get volume info from appSrc
			volSpecPos, err := checkIfVolumeExists(spec.AppFrameworkConfig.VolList, appSrc.VolName)
			if err != nil {
				// Invalid appFramework config.  This shouldn't happen
				return statefulSet, err
			}
			appRepoVol := spec.AppFrameworkConfig.VolList[volSpecPos]

			// Use the provider name to get the corresponding function pointer
			s3Client, err := GetRemoteStorageClient(client, cr, &spec.AppFrameworkConfig, &appRepoVol, appSrc.Location)
			if err != nil {
			...
			appName := appSrc.Name
			appPath := appSrc.Location
			appScope := appSrc.Scope
			initContainerName := fmt.Sprintf(initContainerTemplate, appName, i, appScope)
